...
<Callout>
**Mid-production migration: Supabase → Firebase Auth + Firestore.**
Supabase's free-tier inactivity pausing was a real production risk.
Migrated auth and data while the app was in active use — no downtime,
no data loss — remapping a relational schema to Firestore's
